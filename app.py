...
def student():
    if request.method == "POST":
        ...
        if student_data:
            ...
            if check_password_hash(student_data[4], password):
                login_success = True
            # Check plain text (backward compatibility) and migrate
            elif student_data[4] == password:
                hashed_pw = generate_password_hash(password)
                cursor.execute("UPDATE student SET password = ? WHERE student_id = ?", (hashed_pw, student_id))
                connection.commit()
                login_success = True
                # Refresh data
                cursor.execute("SELECT * FROM student WHERE student_id = ?", (student_id,))
...
def teacher():
    if request.method == "POST":
        ...
        login_success = False
        if teacher_data:
            if check_password_hash(teacher_data[4], password):
                login_success = True
            elif teacher_data[4] == password:
                hashed_pw = generate_password_hash(password)
                cursor.execute("UPDATE teacher SET password = ? WHERE teacher_id = ?", (hashed_pw, teacher_id))
                connection.commit()
                login_success = True
